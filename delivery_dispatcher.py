...
class DeliveryStore:
    ...
    def __init__(self, path: Path, *, max_pending: int = 10000,
                 max_records: int = 100000, max_payload_bytes: int = 65536) -> None:
    ...
    def claim(self, now: float, interval: float) -> Delivery | None:
        with self.db:
            ...
            row = self.db.execute("""
                ...
                WHERE d.status='pending' AND d.due<=? AND t.due<=? AND t.paused=0
    ...
    def complete(self, task: Delivery) -> None:
        with self.db:
...
class DeliveryDispatcher:
    def __init__(self, store: DeliveryStore, send: Callable[[Delivery], Awaitable[None]], *,
                 ...
                 max_attempts: int = 5, retry_base: float = 1, poll_interval: float = 0.05) -> None:
        ...
        try:
            while not stop.is_set():
                ...
                while len(active) < self.concurrency and not stop.is_set():
                    delivery = self.store.claim(time.time(), self.interval)
                    if delivery is None:
                        break
